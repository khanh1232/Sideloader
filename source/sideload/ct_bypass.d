...
void bypassCoreTrust(MachO machO) {
    ...
    // Ép kiểu trực tiếp từng element sang Blob để tránh kích hoạt _d_arraycast của LDC
    ...
    if (machO.filetype == MH_EXECUTE) {
        embeddedSignature.blobs ~= cast(Blob) new DerEntitlementsBlob(entitlements);
    }
    ...
    // Đã sửa đoạn cast mảng bên dưới
    embeddedSignature.blobs ~= [
        codeDir1,
        codeDir2,
        cast(Blob) new TrollSignatureBlob(new CertificateIdentity(X509Certificate(caCertVec, false), RSAPrivateKey(loadKey(source, rng))), [null, sha1, sha2])
    ];
