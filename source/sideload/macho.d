...
// Định nghĩa trực tiếp các cấu trúc Mach-O thuần D
// Giúp LDC không bị kéo các binding Darwin / Objective-C runtime vào khi build Android 32-bit
enum uint MH_MAGIC = 0xffeedfac;
...
class RawBlob: Blob {
    ...
    this(uint type, ubyte[] data) {
        _type = type;
        _data = data[0..std.bitmanip.bigEndianToNative!uint(cast(ubyte[4]) data[4..8])];
    }
...
class CodeDirectoryBlob: Blob {
    ...
    ubyte[] encode(ubyte[][] previousEncodedBlobs) {
        ...
        auto codeDir = new CS_CodeDirectory(
            ...
            0,
            0,
            codeLimit <= uint32_t.max ? cast(uint) codeLimit : 0,
            hashOutputLength,
            ...
            CS_CodeDirectory.sizeof,
            0,
            codeLimit > uint32_t.max ? codeLimit : 0,
            execSegBase,
            execSegLimit,
            execFlags,
        );
...
class SignatureBlob: Blob {
    ...
    ref DEREncoder encodeBlob(return ref DEREncoder der, ubyte[][] codeDirectories) {
        ...
        auto signedAttrs = DEREncoder()
                ...
                .endCons()
                .startCons(ASN1Tag.SEQUENCE)
                    .encode(OID("1.2.840.113549.1.9.5"))
                    .startCons(ASN1Tag.SET)
...
class EmbeddedSignature {
    ...
    ubyte[] encode() {
        ...
        // ĐÃ SỬA ĐOẠN NÀY: Dùng pointer-slice an toàn cho 32-bit thay vì cast(ubyte[])
        return
            ...
            (cast(ubyte*) blobIndexes.ptr)[0 .. blobIndexes.length * BlobIndex.sizeof] ~
            data;
    }
}
...
enum CPU_TYPE_X86 = 7;
enum CPU_TYPE_I386 = CPU_TYPE_X86;
